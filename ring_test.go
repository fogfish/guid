...
func TestOrdRingPrimaryIsLeast(t *testing.T) {
	...
	for i, node := range nodes {
		uids[i] = guid.NewG(guid.NewClock(guid.Mock, guid.WithNodeID(node)))
	}
...
func TestOrdRingKeepsRunsContiguous(t *testing.T) {
	...
	for n, node := range nodes {
		...
		clock := guid.NewClock(guid.Clock, guid.WithNodeID(node), guid.WithClock(func() uint64 { return tick }))
		for i := 0; i < 8; i++ {
...
func TestOrdRingEpochDominates(t *testing.T) {
	early := guid.NewG(guid.NewClock(guid.Mock, guid.WithNodeID(1<<28)))
	late := guid.NewG(guid.NewClock(guid.Clock, guid.WithNodeID(1<<4), guid.WithClock(func() uint64 { return 1 << 60 })))
...
func TestOrdRingRefIsMaskedLikeNodeID(t *testing.T) {
	...
	for i, node := range nodesX {
		uidsX[i] = guid.NewX(guid.NewClock(guid.Mock, guid.WithNodeID(node)))
	}
...
func allocG(t *testing.T, drift guid.Drift, n int) []guid.G {
	...
	for i := 0; i < n; i++ {
		clock := guid.NewClock(guid.Clock, guid.WithDrift(drift), guid.WithNodeID(uint64(i)*0x9E3779B1), guid.WithClock(ticker(drift, i)))
...
func allocX(t *testing.T, drift guid.Drift, n int) []guid.X {
	...
	for i := 0; i < n; i++ {
		clock := guid.NewClock(guid.Clock, guid.WithDrift(drift), guid.WithNodeID(uint64(i)*0x9E3779B97F4A7C15), guid.WithClock(ticker(drift, i)))
